...
import { X, Printer, ShoppingBag, CheckCircle2, ShieldCheck, Monitor } from 'lucide-react';
...
export function OfficeInvoiceModal({ order, onClose }: OfficeInvoiceModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm overflow-y-auto print:p-0 print:bg-white print:static">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-4xl overflow-hidden text-slate-900 print:shadow-none print:border-none print:w-full print:max-w-none">
        ...
        <div className="bg-slate-900 text-white px-6 py-4 flex items-center justify-between print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-sm text-white">Factura de Adquisición de Muebles e Informática</h3>
              <p className="text-xs text-slate-400">Pedido Nº {order.orderNumber} | {new Date(order.purchaseDate).toLocaleString('es-ES')}</p>
            </div>
          </div>
        ...
        <div className="p-8 sm:p-10 space-y-8 print:p-0">
          ...
          {/* Itemized Table */}
          <div>
            <h4 className="font-bold text-xs uppercase tracking-wider text-slate-500 mb-3">DETALLE DE PRODUCTOS ADQUIRIDOS</h4>
